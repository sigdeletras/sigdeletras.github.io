...
Como dejamos reflejado en la entrada [API REST de datos geográficos con Node.js y Express](http://www.sigdeletras.com/2019/apirest-de-datos-geograficos-con-node-y-express/), montar una API REst con Node.js y Express es relativamente sencillo. En el *post*  se desarrollaba un ejemplo para el servicio de una capa que se encontraba almacenada en una basade datos PostgreSQL, a la que se le añadido la extensión PostGIS. En esta ocasión vamos a **mejorar el ejemplo para poder consultar y acceder a cualquiera de las capas que podamos tener en nuestra base de datos y obtener un catálogo sencillo de los datos**. 
...
La primera versión de la GeoAPI permitía obtener el archivo GeoJSON de una capa mediante la URI [http://localhost:3000/api/layers/layer]. Vamos a realizar las modificaciones correspondientes para poder obtener el fichero de cualquiera de las capas almacenadas.
...
```
- Hemos generalizado la sentencia SQL para que nos devuelva todos los campos. El nombre de la tabla será la almacenada en la variable *layername*
```javascript
...
```
- Al generalizar la consulta SQL, se añade también el campo *geom* con la geometría en formato WKT de cada elemento. Este dato no es ncesario,  por lo que usamos un breve código usando la función *omit* del paquete [Underscore.js](https://underscorejs.org/) para eliminarlo de nuestro JSON. El paquete debe ser instalado usando ```npm install underscore -S``` y añadido a nuestro controlador.
...
Ahora vamos a modificar nuestro fichero de rutas **routes/api.js**. Solo debemos añadir dos puntos antes de **layername** para que este valor se convierta en un parámetro en la URL. Este será el valor que usará el controlador dentro de la SQL.
...
Guardamos todos los cambios y reiniciamos el servidor. Podemos usar Postman para probar las nuevas funcionalidades.
